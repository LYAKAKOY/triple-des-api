...
def bin_to_int(binary_string: str) -> str:
...
def shifted(bits: str, shift: int) -> str:
    return bits[shift:] + bits[:shift]


def bin_to_hex(binary: str, length: int) -> str:
    bin_to_hex = {
        "0000": "0",
        "0001": "1",
        "0010": "2",
...
def hex_to_binary(binary: str, length: int) -> str:
    hex_to_bin = {
        "0": "0000",
        "1": "0001",
        "2": "0010",
...
def permute(data, permutation_table, length_of_output) -> str:
    permutation = ""
...
def encrypt(message: str, key: str) -> Dict[str, Any]:
    ...
    steps["data_being"] = bin_to_hex(message, 64)
    perm = IP(message)
    steps["after_initial_permutation"] = bin_to_hex(perm, 64)
    ...
    for loop in range(16):
        ...
        steps[str(loop)] = {
            "round": loop + 1,
            "lefthalf": bin_to_hex(left_half, 32),
            "righthalf": bin_to_hex(right_half, 32),
            "subkey": bin_to_hex(key_schedule[loop], 48),
        }
    cyphertext = IP_Inverse(left_half + right_half)
    steps["result"] = cyphertext
    return steps
...
def decrypt(message: str, key: str) -> Dict[str, Any]:
    ...
    text = IP_Inverse(left_half + right_half)
    steps["result"] = text
    return steps
